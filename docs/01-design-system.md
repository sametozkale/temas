...
- **input.tsx / textarea**: `h-10` (40px touch target), `shadow-none`, focus `ring-1 ring-ring` (not 2px), bg-card. Size `sm` is **32px** (`h-8`, 13px type) for compact toolbars. Select trigger matches input height; the dropdown opens at the trigger’s width.
...
- **tabs.tsx** (property detail tabs): grouped underline tabs matching Tasks — quiet group labels (`text-muted-foreground/50`, Listing / Letting / Record / History) above a single tab row, 24px gap between groups (no vertical rules), sticky `bg-card` hairline `border-b`, active 1.5px `foreground` underline; counts on operational tabs; not pill tabs.
- **dialog/sheet**: radius 16px, hairline border, backdrop `bg-background/80 backdrop-blur-sm`.
- **scrollbars** (`globals.css`): product-wide 8px rounded thumbs, idle `foreground/18` on a transparent track, hover `foreground/28`. `scroll-area` thumb matches. Chrome, not content — quieter than `--border` text.
...
- New components (under components/, not ui/):
  ...
  - `page-header.tsx` — compact serif title (`text-xl`) on one row with actions; description underneath, max-width 42rem; no extra bottom padding (parent spacing).
  ...
  - **Emails**: product mail (`emails/_layout.tsx`) and Auth mail (`supabase/templates/`) share the same chrome — paper `#FAFAF7` canvas, white 10px card, hairline `#E9E6E0`, Newsreader wordmark + heading, Inter body, one warm-black pill CTA (`#22211E`), muted 12px footer. GoTrue cannot read Tailwind, so Auth templates inline those hex values. Hosted projects copy the same HTML into Auth > Email Templates.
...
- **Calendar**: Notion-style month fills the remaining canvas. A 7-column hairline grid (no nested card), weekday labels on a quiet header row, equal-height week rows. Day numbers sit top-left; today is a filled `foreground` circle. Events are **single-line pills** (`h-5`, `rounded-[3px]`, 2px colour bar, `text-[11px]`): time + truncated property title (prospect/agent on the native title). Outside-month cells are a whisper muted. Week is the same 7 columns, taller. List keeps Granola `EventChip` rows. Toolbar: month (or week range) hugging compact chevrons + Today on the left (no reserved month width). Today is omitted while the current day is already on the painted grid (including leading/trailing month cells). Property/agent `sm` selects and Month/Week/List on the right.
- **Home landing**: greeting, composer, coming up and suggestions sit as one block **vertically centred** in the main canvas (not pinned with `pt-[12vh]`). Recent chats live in the sidebar. The empty composer placeholder types and erases example questions.
