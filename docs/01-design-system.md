...
- **checkbox.tsx**: 16px rounded square (`rounded-[5px]`), not a circle. Idle border is `foreground/15` (softer than `--input`); hover `foreground/25`; checked fills `brand` with a light 12px `Tick02Icon` at stroke 2.
...
- **Tasks**: Linear-style issue list. Serif title + primary New task; underline tabs **All / Assigned / Suggested**. Each category is its own block (not one wrapping card): a rounded group bar (property name, count, `+`) with compact rows underneath, and a separate **Completed** bar at the bottom. Rows: 16px Linear priority mark (Low one bar, Medium two, High three, with the unfilled bars at 25% opacity; Urgent is a filled rounded square with the exclamation cut out), all in `foreground`, 16px rounded checkbox status (`Checkbox`, `rounded-[5px]`, idle border `foreground/15`), truncated title, 18px assignee, short date. The priority mark lines up with the group chevron. Icons sit in a 16px slot. Row hover is a `rounded-md` wash. Title, priority and assignee edit inline (click title to type; menus on priority and avatar). Ticking done or reopening moves the row at once; the save finishes behind it. The New task dialog is for create only.
