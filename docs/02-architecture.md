...
- **Inbox is account-owned.** `integrations.user_id` + `unique(user_id, kind)`; `conversations.user_id` is the mailbox owner. RLS: the row is visible only to that user (and they must still be a member of the conversation’s workspace). Settings > Integrations lists the signed-in user’s Google account (mail and calendar from one OAuth grant) and WhatsApp; switching workspace does not share the connection. Connecting Gmail imports the current INBOX (newest 50) before the redirect, and the first Inbox visit retries that import if it did not finish. Later visits show the stored list immediately. When the last pull is older than a minute, new mail and star changes load in the background and the list refreshes only if something changed. The five-minute poll does the same pull. Star checks follow the history feed; a full star sweep runs only when that history has to be rebuilt. Opening a thread shows the stored messages at once and fills in any missing Gmail messages behind that. Each imported thread also stores the mailbox owner’s own messages, so the conversation shows both sides. A new email is composed in the inbox and sent with the Gmail API; the sent message is the first outbound row of a new private conversation. Scrolling the list loads the next older page. Archive, spam, trash, unread, and star on an open email call the Gmail API (`gmail.modify`) and then update `conversations.mailbox_state` / `starred`; the inbox list only shows `inbox`, ordered by the latest message in each thread. Importing an older message does not move that date backwards. Search filters that list by subject, sender name, and sender email. The same Google grant also includes Calendar (`calendar.readonly` and `calendar.events`). The calendar page lists the user’s Google calendars and draws the ones they leave on as a private overlay beside workspace viewings. A right-click writes the chosen event colour back to Google; a repeating event is updated once so every instance stays the same colour.
...
- The commercial unit is the **workspace**. `workspaces.plan` is a catalog key (`free` = Solo, `pro` = Team). Limits and USD prices live in `lib/plans.ts` (seats, listings for monthly and yearly billing, AI credits for monthly and yearly billing, monthly cents, and the monthly cents when billed annually). Solo is a flat price for its one seat ($49 / month, or $29 / month billed annually) with 50 listings, or 100 on the yearly plan, and 500 credits/month, or 750/month when billed annually. Team is per seat, up to 50 seats ($69 / agent / month, or $49 / agent / month billed annually) with 200 listings per agent, or unlimited listings on the yearly plan, and 750 credits/month, or 1,000/month when billed annually. AI quota and the listing cap follow `workspaces.billing_interval`.
- One account on many workspaces does not share a plan: each workspace is billed (later) and gated independently.
- Settings > Billing is owner-only (`billing.manage`): selected catalog plan with a Monthly / Yearly price switch, this-cycle usage, payment method, invoices. The switch stores `workspaces.billing_interval` (`month` or `year`, default month). Current shows only on that interval. The other interval of the same plan offers Switch to yearly or Switch to monthly. Checkout does not charge until `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` are set. Lookup keys live in `lib/billing/catalog.ts`. `POST /api/webhooks/stripe` verifies the signature and writes `stripe_customer_id`, `stripe_subscription_id` and `billing_status` on the workspace. Team quantity is occupied seats. Seat count = members + pending invites. Hard enforcement of seats/listings is deferred.
- Settings > Support is per account (`profiles.support_plan`: `included`, `founder`, `priority`), visible to every role. Included email and the $9 founder WhatsApp / $29 Priority prices live in `lib/support.ts`. The founder number is sent to the page only while Founder or Priority is the saved plan. Choosing a plan stores it without charging until Stripe. Buy for one month is a one-time payment (`support_*_month`); Subscribe is a monthly subscription (`support_*_subscribe`). The webhook writes `support_billing`, `support_period_ends_at`, `stripe_customer_id` and `support_subscription_id` on the profile. Priority notes email the founder address and name the sender, including that they have Priority. Priority also offers the same email and WhatsApp actions.

## 4. Environments & Variables

`.env.example` is mandatory: `DATABASE_URL`, `SUPABASE_*`, `RESEND_API_KEY`, `SEND_EMAIL_HOOK_SECRET`, `FEEDBACK_TO`, `GOOGLE_CLIENT_*`, `META_WHATSAPP_*`, `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `INNGEST_*`, `APP_URL`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`.
