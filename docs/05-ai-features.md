...
- `generateObject` (haiku): `{ tasks: [{ title, description?, priority }], completed: [{ fingerprint?, title? }] }` — `tasks` are concrete **still-open** actions (call the owner, send the contract, chase keys). Empty `tasks` when the thread is chit-chat. `completed` lists existing fingerprints/titles the thread now shows as finished (keys received, contract sent, “thanks / already done”) even if the agent never ticked the row.
