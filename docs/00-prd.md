...
**Plans** are priced **per workspace**, not per account. The same login can be on a Team workspace and a Solo workspace independently. Catalog: `lib/plans.ts` (keys stay `free` = Solo and `pro` = Team). **Solo** is one agent: **$49 / month**, or **$29 / month** when billed annually ($348 a year), with **50** listings (**100** on the yearly plan) and **500** AI credits a month, or **750** a month on the yearly plan. **Team** is per agent, up to **50** seats: **$69 / agent / month**, or **$49 / agent / month** when billed annually ($588 a year per agent), with **200** listings per agent (**unlimited** on the yearly plan) and **750** AI credits a month, or **1,000** a month on the yearly plan. The allowance in force follows the interval saved on the workspace. A normal Ask question spends **1** credit (`lib/ai/credits.ts`); a draft spends 3, an applicant summary 2, and a contract 8. Thread titles, task extract, auto-link, embeddings and reminder copy spend 0. Settings > Billing (owner-only) shows the selected plan at either interval, this-cycle usage, payment method and invoices — the same split as Cursor’s billing dashboard. Current is only the saved interval; the other interval of that plan offers a switch. Checkout does not charge until the Stripe keys are set. Prices are lookup keys in `lib/billing/catalog.ts` (`plan_free_month`, `plan_free_year`, `plan_pro_month`, `plan_pro_year`; Team quantity is occupied seats). The webhook `POST /api/webhooks/stripe` writes `stripe_customer_id`, `stripe_subscription_id` and `billing_status` (`none`, `active`, `past_due`, `canceled`) on the workspace. A deleted subscription returns the workspace to Solo monthly.
...
2. Each task has a **priority** (`low` / `medium` / `high` / `urgent`) and a status (`open` / `done`). Done tasks stay in a collapsed **Completed** group at the bottom of All / Assigned.
3. The main value is **AI suggestions from the signed-in user’s Gmail and WhatsApp threads**. After inbound ingest or an outbound reply, a haiku job extracts a specific unfinished obligation that belongs to that mailbox owner (not the other party’s work, and not a generic “follow up” on every conversation) and writes **private suggestions** (same privacy as Inbox). The mailbox owner accepts (becomes an open, workspace-visible task assigned to them) or dismisses. If a later message in that thread shows the work is finished (keys received, contract sent) and the agent never ticked the row, the job marks that suggested or open task **done**. Colleagues never see another agent’s suggestions or the source thread; only the owner gets a conversation deep link.
...
- Stripe dashboard products, prices and the live webhook endpoint (the catalog, columns and `POST /api/webhooks/stripe` are in the app; the route stays off until `STRIPE_WEBHOOK_SECRET` is set)
