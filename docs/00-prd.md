...
Settings is a gear icon above the user (not a sixth primary item), sitting next to Give feedback and separated from the identity by a hairline. Give feedback opens a dialog; the note is emailed to the product inbox (`FEEDBACK_TO`) via Resend (Mailpit in development). Destination: **Profile** (you + appearance + reply signature), Notifications, AI preferences; **Workspace** general (name, **official company name**, logo, timezone), Members, Integrations (Google for mail and calendar, Outlook later, WhatsApp — the signed-in user’s accounts; each detail page has a **setup view** before it is connected — WhatsApp shows numbered steps and a generated pairing QR), Templates (form + contract), **Billing** (owner: selected plan, usage this cycle, invoices). Stripe checkout is a later phase. Danger zone stays under Workspace general.
